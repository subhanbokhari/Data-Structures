...
public:
    ...
    Node *add(string gem, int power, Node *root)
    {
        if (root == nullptr)
        {
        ...
        if (power > root->power)
        {
            root->right = add(gem, power, root->right);
        }
        else if (power < root->power)
        {
            root->left = add(gem, power, root->left);
        }
...
int main()
{
    ...
    cin >> max_power;
    Tree t1;
    t1.add("Saphire", 70, root);
    t1.add("Pearl", 40, root);
    t1.add("Opal", 10, root);
    t1.add("Garnet", 60, root);
    t1.add("Zircon", 20, root);
    t1.add("Jade", 5, root);
